...
First run: creates `.venv`, installs `vllm==0.27.1` + `flashinfer-python` + `nvidia-cutlass-dsl`, applies the MTP×TurboQuant fix, fetches the model from the shared HF cache (~22 GiB, only if not already there — e.g. via the sglang container), starts the server on port **8888**, and exits once it's healthy and warmed up. Subsequent runs are near-instant if the server is already up.
...
4. **Fetches model** `RadixArk/Qwen3.8-27B-NVFP4` from the shared Hugging Face cache (`$HF_HOME`, default `~/.cache/huggingface`) — the same cache the sglang container mounts, so weights already pulled by sglang are reused as-is; otherwise it downloads there (resumable). vLLM serves directly from the cache dir (unless `MODEL_DIR` points at an existing weights dir).
...
|---|---|---|
| `PORT` | `8888` | API port |
| `MODEL_DIR` | `~/models/RadixArk/Qwen3.8-27B-NVFP4` | existing local weights dir; if absent, weights are served from the shared HF cache instead |
| `HF_HOME` | `~/.cache/huggingface` | shared HF cache (same mount the sglang container uses) |
...
- **Model download interrupted** — just re-run `./start.sh`; `snapshot_download` resumes. If you've ever served this model through the sglang container, the weights are already in the shared `~/.cache/huggingface` and vLLM reuses them — no second download.
...
├── README.md
├── start.sh      # the whole setup: venv → patch → model → serve → warm
