...
#   * fetches the model weights from the shared HF cache (~/.cache/huggingface,
#     the same cache the sglang container mounts) if missing — no re-download
...
# --- 3. model weights --------------------------------------------------------
# Share the Hugging Face cache with the sglang container (which mounts
# ~/.cache/huggingface read/write) so the ~22 GiB weights are fetched at most
# once across both servers. snapshot_download without local_dir resolves
# against this cache: instant no-op if sglang already pulled it, resumable
# download otherwise, and vLLM serves straight from the cache dir.
export HF_HOME="${HF_HOME:-$HOME/.cache/huggingface}"
if [ -f "$MODEL_DIR/config.json" ] && ls "$MODEL_DIR"/*.safetensors >/dev/null 2>&1; then
  log "model present at $MODEL_DIR"
else
  log "fetching $MODEL_REPO from the shared HF cache at $HF_HOME …"
  NEW_DIR="$("$VENV/bin/python" - "$MODEL_REPO" <<'PY'
import sys
from huggingface_hub import snapshot_download
print(snapshot_download(sys.argv[1]))
PY
)" || die "model download failed (re-run resumes)"
  [ -n "$NEW_DIR" ] || die "snapshot_download returned no path"
  MODEL_DIR="$NEW_DIR"
  log "model available at $MODEL_DIR"
fi
